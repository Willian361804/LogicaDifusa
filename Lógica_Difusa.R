...
ui <- fluidPage(
    ...
    # Application title
    titlePanel("Perfil para carreira de cientista de dados"),
    helpText("Configure os sliders e clique em processar"),
    fluidRow(
    ...
    fluidRow(
        ...
        column(4,sliderInput("sestudar","Gosta de estudar",min=20,max=90,step=5,value=40)),
        column(4,sliderInput("scomunica","Habilidade para se comunicar",min=40,max=75,step=5,value=50))
        
    ),
    fluidRow(
        column(6,h1("Sistema:"),plotOutput("Grafsistema")),
        column(6,actionButton("Processar","Processar"),
               helpText("A linha vermelha mostra a sua aderência com a
                        profissão de cientista de dados"),
               plotOutput("GrafResultados")),
...
server <- function(input, output) {
    ...
    observeEvent(input$Processar, {
        inferencia = fuzzy_inference(sistema, list(Gosta_exatas = input$sexatas,
        ...
        output$GrafResultados = renderPlot({
